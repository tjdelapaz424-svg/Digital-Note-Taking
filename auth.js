...
googleOnboardForm.addEventListener('submit', async (e) => {
  ...
  try {
    // The Gmail address IS the account key — same as manual sign-up — so a
    // person can never end up with two separate rows for one email address.
    const emailKey = normalizeGmail(pendingGoogleUser.email);
    const existing = await db.collection('users').doc(emailKey).get();
    if (existing.exists) {
      // Someone else finished this onboarding first (rare race), or a
      // password account with this email was created in the meantime.
      setError('An account with this Gmail address already exists. Try "Continue with Google" again to log in.');
      btn.disabled = false;
    ...
    await db.collection('users').doc(emailKey).set(profile);
    saveSession({ role: profile.role, username: profile.username, usernameKey: emailKey, name: profile.name, authProvider: 'google' });
...
async function finishGoogleSignIn(result) {
  ...
  if (!googleUser.email || !isGmailAddress(googleUser.email)) {
    throw new Error('This Google account does not have a @gmail.com address on file, so it can\'t be used here.');
  }
  // Same account key as manual sign-up (the Gmail address) — this is what
  // lets someone who registered with a password also use "Continue with
  // Google" later and land on the same account, instead of a duplicate.
  const emailKey = normalizeGmail(googleUser.email);
  ...
  // Link the Google identity onto an existing password account the first
  // time it's used, so future admin views clearly show it's the same person.
  if (profile.authProvider !== 'google' || !profile.googleUid) {
    db.collection('users').doc(emailKey).update({ googleUid: googleUser.uid, linkedGoogle: true }).catch(() => {});
  }

  saveSession({ role: profile.role, username: profile.username, usernameKey: emailKey, name: profile.name, authProvider: profile.authProvider });
  window.location.href = profile.role === 'teacher' ? 'teacher.html' : 'student.html';
}
...
loginForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (!data.password) {
      setError('This account was created with "Continue with Google." Use that button instead of a password.');
      return;
    }
...
registerForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (existing.exists) {
      setError('That Gmail address is already registered. Try logging in, or use "Continue with Google" if you originally signed up that way.');
      return;
    }
